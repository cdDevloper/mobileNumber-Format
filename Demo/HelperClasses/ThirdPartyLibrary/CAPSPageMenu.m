...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    /*/[[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(configureUserInterface1)
                                                 name:@"ChangeCapsPageMenuBackground"
                                               object:nil];*/
...
/*- (void)configureUserInterface1:(int)selectedIndex
{
    CGFloat index = 0.0;
    for (UIViewController *controller in _controllerArray) {
        ...
        MenuItemView *menuItemView = _mutableMenuItems [(int)index];
       // AppDelegate *appDelegateObj =  ((AppDelegate *)[[UIApplication sharedApplication] delegate]);
        switch (( int)index) {
            case 0:
                if (selectedIndex == 0) {
                    menuItemView.backgroundColor =  themeColore;
                }else{
                        menuItemView.backgroundColor =  [UIColor colorWithRed:216.0/255.0 green:216.0/255.0 blue:216.0/255.0 alpha:1];
                }
                break;
            case 1:
                if (selectedIndex == 1) {
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (controller.title != nil) {
            ...
            //MARK: - Image of the menu bar title
            //let attachment = NSTextAttachment()
            if (index == 0) {
                
                _imageView1 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.menuScrollView.frame.size.width, self.menuScrollView.frame.size.height)];
                _imageView1.contentMode = UIViewContentModeScaleToFill;
                _imageView1.image = [UIImage imageNamed:@"tab1"];
                [self.menuScrollView addSubview:_imageView1];
                [self.menuScrollView sendSubviewToBack:_imageView1];
                ...
                if ((_currentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation)) || (!_currentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation))) {
                    ...
                    if (page != _currentPageIndex) {
                        ...
                        if (_currentPageIndex == 0) {
                            ...
                            _imageView1 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.menuScrollView.frame.size.width, self.menuScrollView.frame.size.height)];
                            _imageView1.contentMode = UIViewContentModeScaleToFill;
                            _imageView1.image =[UIImage imageNamed:@"tab2"];
                            [self.menuScrollView addSubview:_imageView1];
                            [self.menuScrollView sendSubviewToBack:_imageView1];
                            ...
                            _imageView1 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.menuScrollView.frame.size.width, self.menuScrollView.frame.size.height)];
                            _imageView1.contentMode = UIViewContentModeScaleToFill;
                            _imageView1.image =[UIImage imageNamed:@"tab3"];
                            [self.menuScrollView addSubview:_imageView1];
                            [self.menuScrollView sendSubviewToBack:_imageView1];
                        ...
                        if (!_didTapMenuItemToScroll) {
                           ...
                           //[self configureUserInterface1:(int) _currentPageIndex];
                    }
...
- (void)scrollViewDidEndTapScrollingAnimation {
    ...
    UIViewController *currentController = _controllerArray[_currentPageIndex];
    if ([_delegate respondsToSelector:@selector(didMoveToPage:index:)]) {
       // appDelegate.selectedTabIndex = (int) index;
        //[self configureUserInterface1:(int) _currentPageIndex];
        // [[NSNotificationCenter defaultCenter] postNotificationName:@"ChangeCapsPageMenuBackground" object:nil];
        [_delegate didMoveToPage:currentController index:_currentPageIndex];
    }
